...
`hulubul/core/models/domain/hulubul_models.py` — still fully generated (never hand-edited, a
...
`pyproject.toml`'s `packages` list only includes `hulubul` — so nothing under `hulubul/`
...
hulubul/core/models/domain/hulubul_models.py  # GENERATED (make pydantic), see D9 — Channel, Medium, etc.
```
...
- **One container, both channels.** A single `channel-gateway` service, built from a dedicated
  `infra/docker/channel-gateway/Dockerfile`, sits alongside `langflow`, `postgres`, `neo4j`, and
