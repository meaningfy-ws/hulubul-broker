...
- [ ] 0.1 Relocate `make pydantic`'s output from `model/generated/pydantic/hulubul_models.py`
...
- [ ] 0.2 Update the `check-model-generated` Makefile target's exclusion paths for the new
      location, preserving the current (unverified-reason) exclusion of pydantic output from
      the staleness check.
- [ ] 0.3 Update `AGENTS.md`'s generated-artifacts convention note to cover the new location.
- [ ] 0.4 Remove the old `model/generated/pydantic/` output.
- [ ] 0.5 Add `ADR-019` to `architecture/decisions/README.md` (table row + full entry)
...
- [ ] 1.1 Write `architecture/channel-gateway-blueprint.md` using the `meaningfy-core:technical-writing`
...
- [ ] 1.2 Write `architecture/channel-gateway-runbook.md` using the `meaningfy-core:technical-writing`
...
- [ ] 1.3 Cross-link both new docs from `architecture/README.md` and note the Phase 3
      "prepare later" scope this change pulls forward, per design.md Context.

## 2. Project scaffolding

- [ ] 2.1 Add a new optional Poetry dependency group `gateway` (aiogram 3.x) to
      `pyproject.toml`, following the existing `test`/`quality`/`integration`/`langflow`
      group pattern; run `poetry lock`.
- [ ] 2.2 Create the `src/hulubul/channel_gateway/` package skeleton
      (`models/`, `adapters/`, `services/`, `entrypoints/`, each with `__init__.py`).
- [ ] 2.3 Add the new bounded context to `.importlinter` contracts so `models/` cannot import
...
- [ ] 3.1 Implement `models/message.py`: `InboundMessage`, `TextMessage`, `MediaMessage`
...
- [ ] 3.2 Implement `models/channel.py`: `ChannelRef` value object (medium + systemID only —
...
- [ ] 3.3 Unit tests for both modules in `tests/unit/hulubul/channel_gateway/models/`,
      including the same-channel-same-session_id and different-channels-never-collide
      scenarios from the spec.

## 4. Adapters layer

- [ ] 4.1 Implement `adapters/channel_port.py`: the `ChannelPort` ABC (`receive()`, `send()`),
      per `channel-gateway-adapter` spec "ChannelPort interface".
- [ ] 4.2 Implement `adapters/telegram_adapter.py`: `TelegramAdapter(ChannelPort)` using
...
- [ ] 4.3 Implement `adapters/whatsapp_adapter.py`: `WhatsAppAdapter(ChannelPort)` stub,
...
- [ ] 4.4 Implement `adapters/langflow_client.py`: `LangflowClient` wrapping LangFlow's Run
...
- [ ] 4.5 Unit tests for all four adapters in
...
- [ ] 5.1 Implement `services/relay_message.py`: orchestrates
...
- [ ] 5.2 Add error handling so a LangFlow call failure OR a channel-send failure (e.g.
...
- [ ] 5.3 Unit tests in `tests/unit/hulubul/channel_gateway/services/`, with `ChannelPort` and
      `LangflowClient` mocked.

## 6. Entrypoint

- [ ] 6.1 Implement `entrypoints/telegram_bot.py`: process entrypoint wiring aiogram's
      Dispatcher to `services/relay_message.py`, reading `GATEWAY_MODE` and other
      configuration from environment variables.
- [ ] 6.2 Contract-level test(s) in `tests/unit/hulubul/channel_gateway/entrypoints/`
...
- [ ] 7.1 Identify or add the relevant use case(s) in `architecture/use-cases/` that the
      gateway's Gherkin scenarios should trace back to, per design.md's open question.
- [ ] 7.2 Write `tests/features/telegram_message_relay.feature` (business-language scenarios:
...
- [ ] 7.3 Implement `tests/feature/` step-definitions wiring the `.feature` file to a real
...
- [ ] 8.1 Write `infra/channel-gateway/Dockerfile`.
- [ ] 8.2 Add the `channel-gateway` service to `infra/docker-compose.yaml`
...
- [ ] 8.3 Add the `ngrok` service to `infra/docker-compose.yaml` and wire automatic webhook
      URL registration on gateway startup when `GATEWAY_MODE=webhook`.
- [ ] 8.4 Update `infra/.env.example` with the new required variables.
- [ ] 8.5 Update `AGENTS.md`'s Docker command table if a new `make` target is added for the
      gateway service.

## 9. e2e tests (local/manual only)

- [ ] 9.1 Write `tests/e2e/test_telegram_roundtrip.py`: real Telegram test bot + real local
      LangFlow, full round trip, per `gateway-deployment-setup` spec "e2e suite is excluded
      from CI but documented".
- [ ] 9.2 Write `tests/e2e/test_telegram_adapter_isolation.py`: real Telegram test bot, real
...
- [ ] 9.3 Document how to run `tests/e2e/` locally with a test-bot token in the runbook.

## 10. CI and final verification

- [ ] 10.1 Update CI configuration to run `tests/unit/` and the `tests/feature/`
      step-definitions, explicitly excluding `tests/e2e/`.
- [ ] 10.2 Run `make check-architecture` (import-linter) and confirm no layering violations.
- [ ] 10.3 Run the full unit + feature suite locally and confirm ≥80% coverage on the new
      `channel_gateway` package.
- [ ] 10.4 Manually verify polling mode and webhook mode (via ngrok) both work against a real
      Telegram test bot and the local LangFlow stack, per the runbook.
- [ ] 10.5 Dispatch a separate code-review pass (`meaningfy-building:meaningfy-code-review` /
