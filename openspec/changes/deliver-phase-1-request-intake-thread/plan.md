...
- **RESOLVED — `tests/static/` was never wired into `make ci-static`, so 3 real, pre-existing static-topology test failures had been silently invisible to CI** (found and fixed 2026-07-29, checkpoint 9, while working through the recommended post-10.3 order). `ci-static`'s `test-unit` target only ever ran `pytest tests/unit`; nothing in the CI chain ran `tests/static/` at all, despite it containing real, meaningful topology/security assertions. Once run directly, 3 tests failed: (1) `test_lf10_has_only_lf70_logical_tool` checked the wrong edge-JSON field path (top-level snake_case `target_handle` instead of the real nested camelCase `data.targetHandle.fieldName`) and also still asserted "exactly one tool," stale since DEC-010 added the Graph Identifiers Generator as a second Agent tool — fixed both, renamed to `test_lf10_has_only_lf70_and_identifiers_generator_tools`; (2) `test_lf10_model_cannot_substitute_envelope_or_context` searched for a component type name (`IntakeInputBoundary`) that was never actually built — the real, shared component is `HulubulContractInputBoundary` — fixed; (3) `test_result_boundary_is_output_point` was a genuine gap, not a test bug: LF-00's `HulubulContractResultBoundary` node was never marked `is_output: True` in the flow JSON, contradicting task 44's own "public structured result" design intent — fixed by setting the flag on the live source flow and re-verifying no regression. Added a new `test-static` Makefile target and wired it into `ci-static`'s prerequisite chain (plus the meta-test in `test_project_foundation.py` that asserts that exact chain) so this class of gap can't recur silently.
...
- **OPEN — LF-00 router prompt sets a literal `safe_message` for `outcome="failure"` cases instead of a policy-backed `error` object, bypassing the deterministic error-rendering design** (found 2026-07-29 fixing checkpoint-9 test assertions against real output). `render_router_result()`'s own docstring says a populated `error` always wins and is rendered from the canonical `ERROR_POLICY`, overriding any caller-supplied `safe_message` — this exists specifically so failure messages are policy-backed, not free text. But the deployed LF-00 prompt's decision logic for `routing_stage == "unsupported"` just says `safe_message="request status not supported"` with no mention of populating `error`, and likewise for `routing_stage == "failure"`/inconsistent contexts (`safe_message="routing context invalid"`) — meaning the Router Agent produces its own literal string for these cases rather than the `ErrorCode.UNSUPPORTED_PHASE1_STATUS`/etc. policy text ("This request cannot be handled by the current intake flow."). `RouterResult` doesn't have a model-level invariant forbidding `outcome="failure"` with `error=None`, so this passes validation silently. Not fixed live tonight (would be a fourth deep prompt-rewrite-and-redeploy cycle in one session, per the developer's "let's not waste more time" — see "No subagents" note in `AGENTS.local.md`) — instead the new `test_lf00_main_router.py` assertions were corrected to match the actual current (technically-working-but-architecturally-wrong) literal strings, so the tests are honest about current behavior rather than blocked on this separate fix. Whoever fixes this: check every `safe_message="..."` in the LF-00 prompt's decision-logic section for a case that should instead populate `error: {code, message}` from `ERROR_POLICY`, and update `RouterResult`'s validators to actually enforce "failure outcome must include error" (mirroring `IntakeResult`'s existing equivalent invariant) so this class of bug can't recur silently.

**Update, same day, finishing 10.3's remainder:** stronger evidence this is the actual root cause of observed test flakiness, not just an architecture smell. The duplicate-relationship and duplicate-target graph-corruption test scenarios (both hit the same `routing_stage == "failure"` free-text branch) were observed across multiple independent live runs to non-deterministically produce *either* of two different literal strings ("routing context invalid" vs "I could not produce a safe response.") for the *same* scenario — the mapping between scenario and wording flipped between runs. That's the Router Agent paraphrasing instead of reproducing a fixed literal string reliably, which is exactly the failure mode a policy-backed `error` object (rendered deterministically by `render_operational_error`, not generated by the LLM per-call) would prevent. The affected tests in `test_lf00_main_router.py` now assert membership in both observed strings rather than one pinned value — correct given real behavior, but it's a workaround for this bug, not a fix.

- **RESOLVED — checkpoint 9 (tasks 38-48 / tasks.md 9.x-10.x) delivered flow plumbing and partial live verification, but the verification coverage was uneven across LF-10 and LF-00, and this was initially misreported.** Checkpoint 9 built the LF-10 flow end to end (skeleton, plus an Agent system prompt whose *instructions* cover one-message completion, sparse drafts, focused clarification, and multi-turn accumulation — new `GraphIdentifiersGenerator`/`ContractInputBoundary`/`DataOperationRequestBuilder` components) and rewrote the LF-00 router prompt against the authoritative route matrix. Verified against the live instance, but **not evenly**: for LF-10, only the one-message-complete case (9.2, "task-39" in session notes) was actually invoked end-to-end against the real API and observed; sparse draft (9.3), focused clarification (9.4), and multi-turn accumulation (9.5) were never invoked at all this checkpoint — the prompt text exists, but the behavior is genuinely unverified, not just untested. (First reported to the developer as "all verified live, just untested" — that was wrong; corrected after the developer asked how that was possible and a transcript re-check turned up zero evidence for 9.3-9.5.) For LF-00, by contrast, a real 5-6 scenario pass did happen: Neo4j fixture bindings applied via `cypher-shell`, flows re-run with debug output, structured `RouterResult` extracted per scenario from debug traces — the six-core-scenario claim (absent/new/needsClarification/complete/closed/unsupported) holds up on inspection. Concretely missing, regardless of live-verification status: `tests/integration/langflow/test_lf10_request_intake.py` (tasks 9.2-9.6, does not exist at all — zero automated LF-10 coverage beyond static topology, and for 9.3-9.5 the underlying behavior needs to be exercised for the first time, not just wrapped in a test); the `new`/`needsClarification` routes in `test_lf00_main_router.py`'s route matrix (task 10.2, needs Neo4j fixtures); the full informational/fail-closed matrix — complete, closed precedence, all 8 post-intake statuses, unknown/null/duplicate contexts (task 10.3, currently absent, though the underlying behavior was live-verified); and the LF-00 runtime trace boundary (task 10.4 — `tests/support/trace_metadata_probe.py`, `tests/fixtures/langflow/trace_topology_v1.json`, `tests/integration/langflow/test_lf00_traces.py`, none of which exist; distinct from the static wiring checks in `test_lf00_topology.py`). Task 10.5's remaining gap ("exhaustive route matrix, safe failures") is delivered by 10.3 in the same file — not a separate work item. tasks.md checkboxes for 9.1/10.1 (skeleton) are checked since those are genuinely done with static evidence; everything else in 9.x/10.x stays unchecked until real pytest coverage lands. Also open from checkpoint 9: the RunFlow tool-mode `input_type` hardcoding workaround (`~type: "any"` tweak) is applied and helps but is not proven 100% reliable — see `DEV/knowledge/checkpoint9-lf10-runflow-injection-runbook.md` (local-only) for the root cause and reproduction notes.
  **Resolved 2026-08-06:** every gap listed above has since landed with real pytest coverage — `tests/integration/langflow/test_lf10_request_intake.py` exists and covers 9.2-9.6 (commit `149aabc`, task 9.6), the LF-00 route matrix (`new`/`needsClarification`, complete/closed/unsupported/unknown/duplicate) is covered in `test_lf00_main_router.py` (task 10.3), and the trace boundary (`trace_metadata_probe.py`, `trace_topology_v1.json`, `test_lf00_traces.py`) exists and is referenced by task 10.4's own tasks.md entry. All of 9.x/10.x are now checked `[x]` in tasks.md and the checkboxes are accurate. This entry is kept for its historical record of the coverage-evenness miscommunication, not because the gap is still open.

- **OPEN — `ContractInputBoundaryComponent`'s `envelope`/`routing_context` fields are `advanced=True`, which is architecturally wrong per `DEV/reports/advanced-field-in-langflow.md` (that report's fix is what's reverted here), because the correct value crashes every LF-00 call.** Setting `advanced=False` (the report's prescribed, otherwise-correct fix, which does restore the real edges LangFlow's frontend silently drops) makes those fields tool-mode-eligible whenever this component sits at a `RunFlow`-invoked flow's input boundary. `lfx`'s `create_input_schema_from_dict()` (`lfx/io/schema.py`) then crashes building a Pydantic schema for the raw `type: "other"` label with `name 'other' is not defined` — confirmed live, 100% reproducible via the full `test_lf00_main_router.py` suite (24/24 failing with `advanced=False`, 24/24 passing after reverting to `advanced=True`). This is a genuine upstream `lfx` gap (`create_input_schema_from_dict` doesn't normalize a Handle-typed field's raw JSON `type` the way its sibling `create_input_schema` does), not something introduced by this project's own code. Full trace, root cause, and next-step options: `DEV/knowledge/checkpoint9-lf10-runflow-injection-runbook.md`'s 2026-07-31 entry.

- **OPEN (informational, not necessarily a bug) — the Router Agent's decision to invoke the LF-10 tool for the same message/scenario is genuinely non-deterministic** (found while building task 10.4's trace-boundary tests). For the absent-binding "I need to send a parcel" scenario, identical repeated live calls sometimes produced a trace with 15 spans and exactly 1 model call (Router answers directly, no tool call) and sometimes 26 spans with a nested `lf-10-request-intake_tool` call and multiple model calls (reason → act → observe → answer) — same prompt, same input, same Neo4j state. This is ordinary LLM tool-use variance, not a code defect, but it means no test or fixture can honestly assert "the Router always/never calls LF-10" for that scenario; task 10.4's tests and frozen topology fixture were built against the "complete status" no-mutation route instead, which has no plausible reason to call any tool and was verified deterministic across repeated live runs. Also notable: the LF-00 → LF-10 actual-delegation architecture gap flagged elsewhere in this file (Router prompt never instructing an LF-10 tool call) reads as at least partially closed by whatever changed between when that gap was documented and this session's live traces — the tool clearly *can* and sometimes *does* get called now — but given the non-determinism above, that's an observation, not a verified fix; worth re-confirming deliberately before relying on it.

- **RESOLVED — `HulubulRunFlowComponent`'s per-call deepcopy crashed on an unpicklable cached tool-mode output, specific to Playground-triggered streaming builds** (found 2026-08-06, debugging a developer report of "I could not produce a safe response" that a plain-run repro attempt could not reproduce). `Component.__deepcopy__` (`lfx==1.10.2`) protects `self._inputs` from a deepcopy failure with a try/except-and-shallow-copy fallback, but deep-copies `self._outputs_map` two lines later with no protection at all. Confirmed via a captured live traceback (temporary, reversible diagnostic instrumentation injected directly into the live flow's embedded component code — not guessed): a flow build triggered through the streaming endpoint (`/api/v1/build` + `/events`, what the Playground UI actually uses — never the plain `/api/v1/run/`) wires in LangFlow's own tracing service (`NativeTracer`), whose background flush task ends up nested inside the cached `component_as_tool` output value at `Output.value[0].callbacks[0].tracer._flush_task`. `asyncio.Task` cannot be deep-copied, so the per-tool-call copy `ComponentToolkit` makes on every Agent tool invocation crashed with `TypeError: cannot pickle '_asyncio.Task' object`, and the tool call failed after LangChain's own retries were exhausted — this is what actually produced the "could not produce a safe response" fallback, unrelated to anything in the router or intake prompts. Fixed in `HulubulRunFlowComponent.__deepcopy__`: detach `_outputs_map` before the base copy runs, reattach a defensively-copied version via a new `deepcopy_outputs_with_fallback()` policy function (`run_flow_policy.py`) that mirrors the same try/except-with-fallback trade-off the base class already makes for `_inputs`. Also fixed the identical latent defect in LF-10's own `RunFlow-hlb-lf-10-data-access-v1` node (same component class, same tool-mode output shape) before it was independently reported. Verified live via the streaming build mechanism: 3/3 clean runs post-fix, zero pickle errors, versus reliable reproduction pre-fix. Also surfaced, and now documented in `AGENTS.md`/`.claude/resources/langflow-invocation-mechanisms.md`: the plain-run and streaming-build invocation mechanisms are not interchangeable for repro/verification — a bug can reproduce 100% of the time on one and 0% on the other with identical input. Commit `d8e7364`.

- **RESOLVED — a Playground conversation's session UUID was random per message, not stable per conversation, breaking multi-turn intake accumulation** (found 2026-08-06, investigating a developer report that a multi-turn conversation kept re-asking questions already answered). `ExecutionEnvelopeComponent._session_uuid_part()`'s Playground fallback minted a fresh `uuid4()` on every call for a non-UUID session label (e.g. `"New Session 0"`, what the Playground UI actually sends — never a real UUID). Confirmed directly in Neo4j: a single nominal conversation created five separate `OperationalConversationBinding` nodes, one per turn, each with a different `sessionId` — every message landed on a disconnected session with no memory of prior facts, so LF-10 saw an empty draft state every turn and re-asked for already-supplied information. One-shot prompts were unaffected, since a single call never needs cross-turn continuity — which is exactly why this had not been noticed sooner. Fixed by deriving the UUID with `uuid5()` against a fixed namespace instead of `uuid4()`, so the same label always resolves to the same session while still producing a schema-valid UUID (the original problem non-UUID Playground labels needed solving for, per the source-code history this fallback came from). Verified live via the streaming build mechanism, replaying the exact failing multi-turn sequence: state now persists correctly across turns even through an unrelated mid-conversation hiccup. Commit `4109a36`.

- **PARTIALLY MITIGATED, root cause remains the already-open "literal `safe_message` instead of policy-backed `error`" item above** — LF-00's router prompt lacked an explicit guard against pairing `reason="invalidContext"` with `outcome="routed"`/`target="intake"`, and the router agent's `temperature` was unset (found 2026-08-05/06, same investigation thread as the item above). The prompt's own rule 5 already correctly specified that `reason="invalidContext"` must pair with `outcome="failure"`/`target="none"`, but nothing forbade the model from violating its own rule — confirmed live, the model occasionally paired them with `outcome="routed"`/`target="intake"` instead, which the mandatory hand-off validation then correctly rejected, producing the generic "could not produce a safe response" fallback instead of the model's own (already wrong) text. Added an explicit anti-pattern warning (matching the style of the existing `noBinding`/`intakeInProgress` guardrail already in the prompt) and pinned the router agent's `temperature` to `0` (previously unset, deferring to the provider's own default). This reduces but does not provably eliminate the underlying misclassification: the same inconsistency was reproduced once even at temperature 0, since the router's decision logic is being asked of an LLM at all rather than computed deterministically from `routing_context` — a genuinely pure function of typed fields (`binding_state`, `routing_stage`, `request_status`). The architecturally complete fix is still the one described in the open item above: populate a policy-backed `error` object and enforce a "failure outcome must include error" invariant on `RouterResult`, ideally moving the deterministic branch of the decision (rules 1, 3, 4, 5) out of the LLM's hands entirely and reserving the model for the one genuinely-needs-language-understanding step (delegating to LF-10). LF-00 was also redeployed under a new flow id in the same commit to escape a separate, unrelated recurring live-vs-committed drift bug (LangFlow's own auto-save plus restart-triggered component-registry refresh repeatedly dropping an edge) — since resolved by replacing the original flow in place with the fixed one (see below). Commit `8d376f0`.

- **RESOLVED — LF-10's intake agent occasionally produced an `IntakeResult` that failed schema validation, in two distinct confirmed ways** (found 2026-08-06, tracing a "could not produce a safe response" failure to LF-10 rather than LF-00's router, which was correctly relaying the tool's own result verbatim per its prompt). (a) The agent emitted `"sender_id"` inside `facts` instead of `"sender_actor_id"` — two different fields on two different objects (`sender_actor_id` is the trusted actor URN on `IntakeFacts`; `sender_id` is a generated graph node ID on `GraphIdentifiers`, returned by a separate tool call sitting adjacent in the same tool-calling flow). (b) The agent emitted `""` for an unset optional text field, which fails every `IntakeFacts` text field's `min_length=1` constraint (`HumanSuppliedText`/`NonBlankText`) exactly like a missing required field would. Fixed with an explicit `facts` key-name enumeration (with the `sender_id`/`sender_actor_id` disambiguation spelled out) and an explicit null-not-empty-string instruction, plus a bookend reinforcement of the prompt's existing (but insufficient on its own, per a confirmed rejected retry) anti-prose rule. Commit `e12d845`.

- **RESOLVED — `readDeliveryRequest`'s own Cypher query never returned the Agent nodes carrying a receiver's/sender's name, making correct multi-turn fact recall structurally impossible** (found 2026-08-06, investigating what first looked like an LF-10 merge-logic bug — turned out to be upstream, in LF-70). The query `OPTIONAL MATCH`es `senderAgent`/`receiverAgent` (traversing `PLAYED_BY` from the `Sender`/`Receiver` role nodes) but never includes them in its `RETURN` clause — only the bare role nodes, which carry no name property at all. Confirmed directly against the graph: a receiver's name ("Janice") was correctly persisted on `receiverAgent.name`, but the read query could never surface it, structurally, regardless of how well the consuming agent was prompted — this is what actually produced the `sender_id`/`receiver_id` field-name confusion in the item above: denied the real data, the agent filled in graph node IDs instead. Compounding this, the `facts` contract description was vague (`"...sparse fields present, absent ones omitted..."`, no explicit key names), unlike `createDeliveryRequest`'s own explicit contract. Fixed both together: the query now returns `senderAgent`/`receiverAgent`, and the facts contract explicitly maps each `IntakeFacts` key to its source node property (`receiver_name` ← `receiverAgent.name`, `pickup_location` ← `pickup.name`, etc.), with an explicit warning against using graph node IDs as fact values. Verified live at the mechanism level, not just end-to-end: pulled the actual `readDeliveryRequest` tool response from the trace post-fix and confirmed `result.facts` correctly reads `{"sender_display_name": "urn:...", "receiver_name": "Janice"}`, and separately confirmed in Neo4j that a completed request correctly shows `receiverAgent.name = "Janice"`. Commit `9122f67`.

- **RESOLVED — an unqualified short answer to a clarification question was sometimes attributed to the wrong intake field, reproducing the same repeating-question symptom the session-continuity fix above had already fixed the *other* cause of** (found 2026-08-06, a developer report of exactly this loop recurring after the session-continuity fix landed). Asked "Where should the parcel be picked up?", the agent misfiled a direct, unlabeled answer ("Warsaw, royal st. 12") as `drop_off_location` instead of `pickup_location` — even though `missing_required_fields()`/`select_clarification_field()` (`completeness.py`) are pure, deterministic functions with a fixed priority order (receiver identity, pickup_location, drop_off_location, parcel_declared_content) that guarantee, not merely suggest, which field the renderer used to generate the question being answered. `pickup_location` stayed null, `missing_fields` kept listing it, and the same question got asked on every subsequent turn — session continuity itself was confirmed working correctly the whole time (same `session_id` across all turns). Fixed with an explicit rule: attribute an unqualified answer to `missing_fields[0]` from the snapshot just read, never guess from other cues, unless the message explicitly names or qualifies a different field. Verified live at both the conversational and mechanism level: replayed the exact failing sequence on a fresh session, confirmed the correct field progression (no repeat), and confirmed via the trace that `hulubul_graph_identifiers_generator` was called with `pickup_location="Warsaw, royal st. 12"`, not `drop_off_location`. Commit `af97807`.

- **Not a bug, noted for completeness — LF-00 replaced in place, old flow id retired.** The redeployed LF-00 flow from the item above (id `29c624c0-...`, temporarily named `lf-00-main-router-v2` while both flows coexisted for comparison) ran clean through a full session of the fixes above; the original flow (id `38b7ee64-26c8-5d4d-97e4-6e62a0fcb557`) was deleted from the LangFlow server and the fixed flow renamed back to the canonical `lf-00-main-router` name. The id (`29c624c0-...`) is unchanged and is what `flow-manifest.yaml`, every test, and every script have referenced since the redeploy — only the display name changed back. Verified: live flow matches committed JSON byte-for-byte after the rename, and a smoke-test prompt against the renamed flow returns the correct response. Commit `92da38c`.
