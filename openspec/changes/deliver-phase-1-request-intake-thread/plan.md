...
- **OPEN — checkpoint 9 (tasks 38-48 / tasks.md 9.x-10.x) delivered flow plumbing and live verification, but automated evidence for 9.2-9.6, 10.2 (partial), 10.3, and 10.4 was never written.** Checkpoint 9 built the LF-10 flow end to end (skeleton, one-message completion, sparse drafts, focused clarification, multi-turn accumulation — new `GraphIdentifiersGenerator`/`ContractInputBoundary`/`DataOperationRequestBuilder` components) and rewrote the LF-00 router prompt against the authoritative route matrix, verifying all of it by hand against the live local LangFlow instance (curl/MCP tool calls, live payload replay against LF-70, confirmed real Neo4j writes). That live verification was mistaken for "task done" and work moved on to task 48 without circling back to write the pytest evidence tasks 39-43 and 46-47 actually call for. Concretely missing: `tests/integration/langflow/test_lf10_request_intake.py` (tasks 9.2-9.6, does not exist at all — zero automated LF-10 coverage beyond static topology); the `new`/`needsClarification` routes in `test_lf00_main_router.py`'s route matrix (task 10.2, needs Neo4j fixtures); the full informational/fail-closed matrix — complete, closed precedence, all 8 post-intake statuses, unknown/null/duplicate contexts (task 10.3, `TestNoMutationRouteMatrix`-shaped, currently absent); and the LF-00 runtime trace boundary (task 10.4 — `tests/support/trace_metadata_probe.py`, `tests/fixtures/langflow/trace_topology_v1.json`, `tests/integration/langflow/test_lf00_traces.py`, none of which exist; this is distinct from the static wiring checks in `test_lf00_topology.py`). tasks.md checkboxes for 9.1/10.1 (skeleton) are checked since those are genuinely done with static evidence; everything else in 9.x/10.x stays unchecked until real pytest coverage lands. Also open from checkpoint 9: the RunFlow tool-mode `input_type` hardcoding workaround (`~type: "any"` tweak) is applied and helps but is not proven 100% reliable — see `DEV/knowledge/checkpoint9-lf10-runflow-injection-runbook.md` (local-only) for the root cause and reproduction notes.
