...
- **LF-70 write-operation decision-level nondeterminism** (found during tasks 34-36/8.3-8.5 verification): a well-formed, schema-valid `DataOperationResult(outcome="rejected", success=false)` was observed for scenarios with a valid compare-and-set match that should have confirmed, on both `updateDeliveryRequest` and `setRequestStatus`; later reruns of the identical scenarios succeeded cleanly. Confirmed to be a property of this model/flow's write path in general, not one operation's Cypher or prompt section. This is a distinct failure mode from the JSON parse/validation failures that `LangFlowClient.run_flow_with_actor`'s bounded retry now covers — retrying on `success: false` generically would be wrong (it would also retry genuine rejections, e.g. a real duplicate-binding conflict). Tracked as tasks.md 14.1; full investigation notes in `DEV/knowledge/checkpoint8-lf70-troubleshooting-runbook.md`, bug #17.
