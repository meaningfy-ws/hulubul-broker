...
- [ ] 9.2 Implement one-message complete intake from trusted Sender context through complete graph persistence and confirmed `new -> complete` transition — the only one of 9.2-9.5 actually invoked end-to-end against the live LF-10 API this checkpoint (direct `POST /api/v1/run/<lf10-id>`, complete-facts payload); `tests/integration/langflow/test_lf10_request_intake.py` does not exist, so no automated evidence, but the behavior itself was observed working once
- [ ] 9.3 Implement sparse draft capture that persists every valid supplied fact, creates no placeholder domain nodes, and confirms `new -> needsClarification` — **never invoked, live or otherwise, this checkpoint.** The Agent's system prompt contains written instructions for this case ("For no binding create one sparse truthful request/binding...") but that instruction has never actually been exercised against the running flow. Not "tests missing" — behavior is unverified, full stop.
- [ ] 9.4 Implement focused clarification selection that reports the complete missing set while asking for exactly one field — **never invoked**, live or otherwise, this checkpoint. Same caveat as 9.3: prompt text exists, behavior unverified.
- [ ] 9.5 Implement multi-turn accumulation against the same sparse snapshot, immediate validation on each turn, optimistic updates, and final `needsClarification -> complete` — **never invoked**, live or otherwise, this checkpoint. Same caveat as 9.3.
