...
- [ ] 14.1 Investigate LF-70 write-operation decision-level nondeterminism (createDeliveryRequest/updateDeliveryRequest/setRequestStatus): a well-formed, schema-valid `DataOperationResult(outcome="rejected", success=false)` was observed for scenarios that should have confirmed (valid compare-and-set match), with unrelated reruns of the identical scenarios then succeeding cleanly. Reproduced on both updateDeliveryRequest and setRequestStatus, confirming it's a property of this model/flow's write path in general, not one operation's Cypher or prompt section. Distinct from, and not fixed by, the parse-failure retry added to `LangFlowClient.run_flow_with_actor` (tasks 8.3-8.5 work). Needs a live-traced reproduction (see the reusable Agent-tracing technique) to see whether the model is mis-reading its own tool result or second-guessing a correct match, plus a product decision on whether an occasional false-rejection is tolerable for Phase 1 or needs a stronger guardrail. Full details: `DEV/knowledge/checkpoint8-lf70-troubleshooting-runbook.md`, bug #17.
