...
(Canonical Make Targets) adds the namespaced Python/CI/acceptance Make
targets without changing the pre-existing ``lint`` (LinkML-only) target. The
Makefile is the single task-runner surface (tox.ini was removed — its
default env list and opt-in live-model evaluation env are now the
``test-evaluation-recorded``/``test-evaluation-live`` Make targets).
"""
...
def test_evaluation_targets_exist_and_are_marker_scoped(makefile_text: str) -> None:
    recorded = target_body(makefile_text, "test-evaluation-recorded")
    live = target_body(makefile_text, "test-evaluation-live")
    assert "-m evaluation" in recorded
    assert '-m "evaluation and live_model"' in live
