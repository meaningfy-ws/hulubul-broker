...
class TestClarificationQuestions:
    ...
    def test_covers_every_intake_field(self):
        assert set(CLARIFICATION_QUESTIONS.keys()) == set(IntakeField)

    def test_exact_canonical_wording(self):
...
class TestRenderClarificationMessage:
    ...
    def test_renders_exactly_one_question_per_field(self, field):
        text = render_clarification_message(field)
        assert text == CLARIFICATION_QUESTIONS[field]
        assert text.count("?") == 1

    def test_receiver_identity_exact_wording(self):
...
class TestRenderCompleteMessage:
    ...
    def test_different_request_ids_render_different_text(self):
        assert render_complete_message("req-1") != render_complete_message("req-2")

    def test_never_mentions_a_missing_field(self):
        text = render_complete_message("req-123")
...
class TestRenderOperationalError:
    ...
    def test_matches_canonical_policy_message(self, code):
        error = make_error(code)
        assert render_operational_error(error) == ERROR_POLICY[code].safe_message

    def test_never_leaks_raw_internal_text(self):
...
class TestRenderIntakeResult:
    ...
    def test_clarification_renders_exactly_selected_question(self):
    ...
    @pytest.mark.parametrize("field", list(IntakeField))
    def test_clarification_matches_canonical_map_for_every_field(self, field):
        result = clarification_result(field)
        assert render_intake_result(result) == CLARIFICATION_QUESTIONS[field]

    def test_complete_includes_confirmed_request_id(self):
        result = IntakeResult(
    ...
    def test_failure_delegates_to_pre_validated_safe_error(self):
        result = IntakeResult(
...
class TestRenderRouterResult:
    ...
    def test_error_present_delegates_to_canonical_safe_error(self):
    ...
    def test_no_error_returns_validated_safe_message(self):
        result = RouterResult(
            correlation_id=uuid4(),
    ...
    def test_informational_outcome_returns_validated_safe_message(self):
        result = RouterResult(
            correlation_id=uuid4(),
...
class TestRenderStatusUpdate:
    ...
    def test_complete_status_exact_wording(self):
    ...
    def test_every_message_is_human_prose_ending_in_a_period(self):
