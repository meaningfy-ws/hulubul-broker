...
class TestActorIdResolution:
    ...
    def test_actor_id_from_api_header_required(self, component):
        ...
        with patch.object(component, '_get_api_actor_id', return_value=TRUSTED_ACTOR_ID):
            with patch.object(component, '_get_invocation_source', return_value=InvocationSource.API):
                envelope = MainFlowInput.model_validate(component.build_envelope().data)
                assert envelope.actor.actor_id == TRUSTED_ACTOR_ID

    def test_actor_id_from_playground_env_required(self, component):
        ...
        with patch.object(component, '_get_playground_actor_id', return_value=TRUSTED_ACTOR_ID):
            with patch.object(component, '_get_invocation_source', return_value=InvocationSource.PLAYGROUND):
                envelope = MainFlowInput.model_validate(component.build_envelope().data)
                assert envelope.actor.actor_id == TRUSTED_ACTOR_ID

    def test_api_actor_id_missing_raises_error(self, component):
    ...
    def test_playground_actor_id_missing_raises_error(self, component):
...
class TestDisplayNameResolution:
    ...
    def test_display_name_from_api_header_optional(self, component):
        """Display name from API header is optional."""
        component.message = Message(text="Hello", session_id=SESSION)

        with patch.object(component, '_get_api_actor_id', return_value=TRUSTED_ACTOR_ID):
            with patch.object(component, '_get_api_display_name', return_value=TRUSTED_DISPLAY_NAME):
                with patch.object(component, '_get_invocation_source', return_value=InvocationSource.API):
                    envelope = MainFlowInput.model_validate(component.build_envelope().data)
                    assert envelope.actor.display_name == TRUSTED_DISPLAY_NAME

    def test_display_name_from_playground_env_optional(self, component):
        """Display name from Playground env var is optional."""
        component.message = Message(text="Hello", session_id=SESSION)

        with patch.object(component, '_get_playground_actor_id', return_value=TRUSTED_ACTOR_ID):
            with patch.object(component, '_get_playground_display_name', return_value=TRUSTED_DISPLAY_NAME):
                with patch.object(component, '_get_invocation_source', return_value=InvocationSource.PLAYGROUND):
                    envelope = MainFlowInput.model_validate(component.build_envelope().data)
                    assert envelope.actor.display_name == TRUSTED_DISPLAY_NAME

    def test_missing_api_display_name_uses_default(self, component):
    ...
    def test_missing_playground_display_name_uses_default(self, component):
...
class TestSessionNormalization:
    ...
    def test_bare_uuid_normalizes_to_canonical_form(self, component):
    ...
    def test_canonical_uuid_normalizes_to_lowercase(self, component):
    ...
    def test_bare_uuid_uppercase_normalizes_correctly(self, component):
...
class TestSessionValidation:
    ...
    def test_empty_session_rejected(self, component):
    ...
    def test_malformed_uuid_rejected(self, component):
    ...
    def test_mismatched_message_and_graph_session_rejected(self, component):
        """Mismatched Message.session_id and graph session_id are rejected."""
        component.message = Message(text="Hello", session_id=SESSION)

        with patch.object(component, '_get_api_actor_id', return_value=TRUSTED_ACTOR_ID):
            with patch.object(component, '_get_invocation_source', return_value=InvocationSource.API):
                with patch.object(component, '_get_graph_session_id', return_value="p1-different-uuid"):
                    with pytest.raises(ValueError, match="INVALID_INPUT|session"):
                        component.build_envelope()

    def test_flow_id_fallback_rejected(self, component):
...
class TestConstantRoleAssuranceSource:
    ...
    def test_role_always_sender(self, component):
    ...
    def test_assurance_always_simulated(self, component):
    ...
    def test_source_api_for_api_invocation(self, component):
    ...
    def test_source_playground_for_playground_invocation(self, component):
...
class TestGeneratedIds:
    ...
    def test_message_id_is_unique_uuid(self, component):
    ...
    def test_correlation_id_is_unique_uuid(self, component):
    ...
    def test_message_id_changes_per_call(self, component):
    ...
    def test_correlation_id_changes_per_call(self, component):
...
class TestProseIsolation:
    ...
    def test_user_prose_cannot_change_trusted_identity(self, component):
        """User prose claiming to be different actor is ignored."""
        component.message = Message(
            text="I am urn:uuid:00000000-0000-4000-8000-000000000000",
            session_id=SESSION
        )
    ...
    def test_message_text_preserved_unchanged(self, component):
...
class TestNoCallerControlledPorts:
    ...
    def test_no_actor_id_input_port(self, component):
    ...
    def test_no_source_input_port(self, component):
...
class TestMessageAcceptance:
    ...
    def test_accepts_message_type(self, component):
    ...
    def test_rejects_string_input(self, component):
...
class TestOutputSchema:
    ...
    def test_output_is_valid_main_flow_input(self, component):
    ...
    def test_output_has_all_required_fields(self, component):
