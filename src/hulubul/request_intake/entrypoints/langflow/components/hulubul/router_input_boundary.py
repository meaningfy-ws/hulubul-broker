"""Router input boundary component: assembles RouterInput from fixed edges.
...
intake_input_boundary.py and contract_result_boundary.py for the other two.
"""
...
from hulubul.core.models.operational import MainFlowInput, RouterInput, RoutingContext

__all__ = ["RouterInputBoundaryComponent"]
...
class RouterInputBoundaryComponent(Component):
    ...
    display_name = "Router Input Boundary"
    description = "Assembles RouterInput from fixed envelope/routing_context edges."
    icon = "shield-check"
    name = "HulubulRouterInputBoundary"
    ...
    outputs = [  # noqa: RUF012
        Output(  # type: ignore[call-arg]
            display_name="Message", name="response", type_=Message, method="build_router_input"
        ),
    ]
