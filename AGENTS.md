...
LangFlow exposes two distinct ways to execute a flow, and they are **not
equivalent** — a bug can reproduce on one and never on the other, even with
identical input:
...
drawing either conclusion. Minimal repro:
...
For a multi-turn conversation, run turns **sequentially** (wait for one
turn's events stream to finish before sending the next) — firing multiple
`/build` calls back-to-back without waiting does not reproduce a real
conversation and can interleave state incorrectly.
